@echo off
color 3
del /s /q "Clients\2011E\content\scripts\host.lua"
del /s /q "Clients\2011E\content\scripts\join.lua"
del /s /q "Clients\2011E\content\scripts\solo.lua"
title Roblox Server Tool 2011E
cls

echo Buildium Server tool (2011E) - Written by Disasterocks
echo.
echo 1. Host
echo 2. Join
...
echo local suc, err = pcall(function() >>join.lua
echo dofile("rbxasset://scripts\\CSMPFunctions.lua") >>join.lua
echo dofile("rbxasset://scripts\\FixAssetLinks.lua") >>join.lua
echo game:GetService("Players"):SetChatStyle(Enum.ChatStyle.ClassicAndBubble) >>join.lua
...
echo game:Load('rbxasset://gui.rbxm') >>join.lua
echo game:FindFirstChild("HealthGUI").Parent = game.StarterGui >>join.lua
echo server:start(port, 20) >>host.lua
echo dofile('rbxasset://scripts\\CSMPFunctions.lua') >>host.lua
echo dofile('rbxasset://scripts\\FixAssetLinks.lua') >>host.lua
echo game:getService('Players'):SetChatStyle(Enum.ChatStyle.ClassicAndBubble) >>host.lua
echo.
